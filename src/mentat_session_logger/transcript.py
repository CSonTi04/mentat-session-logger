...
class SpeakerMapApplicationStage:
    ...
    def run(self, context: SessionContext, artifacts: ArtifactStore) -> StageResult:
        ...
        return StageResult(
            input_artifacts=[ArtifactRef("diarized_raw", source), ArtifactRef("speaker_map", mapping_path)],
...
class GlossaryCorrectionStage:
    ...
    def __init__(self, llm_client: LlmClient, prompt_renderer: PromptRenderer, workspace_root: Path) -> None:
    ...
    def run(self, context: SessionContext, artifacts: ArtifactStore) -> StageResult:
        ...
        glossary = read_text(glossary_path) if glossary_path.exists() else read_text(fallback_glossary)
        ...
        try:
            corrected = self.llm_client.generate(prompt + "\n\nGlossary:\n" + glossary + "\n\nTranscript:\n" + transcript)
